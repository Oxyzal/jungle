...
const showPlat = (title) =>{
    ...
    menu.forEach(element => {
        if(element.title === title){
            ...
            div.innerHTML +=`<p data-food='${element.desc}' class='food ${element.title}'> ${element.desc} </p>`
            div.innerHTML +=`<p data-food='${element.price}' class='food price'> ${element.price} </p>`    
        }
    })
}
